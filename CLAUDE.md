...
1. `gatsby-config.ts` が `articles/` 配下のサブディレクトリ（`tech`, `other`, `blog`, `category`、ファイルシステム上は `daily`, `book` もあるが `gatsby-config.ts` に登録されているもののみ取り込まれる）を **別々の `gatsby-source-filesystem` インスタンス** として登録し、それぞれの `name` を `sourceInstanceName` として保持する。
